...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='/home/DataBase2/sunpeiwen/dataset/RAF-DB/basic/', help='Raf-DB dataset path.')
...
def run_training():
    ...
    if args.backbone=="resnet18":
        res18 = resnet18(pretrained = imagenet_pretrained, drop_rate = args.drop_rate, args=args)
    elif args.backbone=="iresnet18":
        res18 = iresnet18(pretrained = imagenet_pretrained, drop_rate = args.drop_rate, args=args)
    res18 = torch.nn.DataParallel(res18).cuda()
    ...
    if args.pretrained:
        ...
        if args.backbone=="resnet18":
            ...
            loaded_keys = 0
            total_keys = 0
            temp_pretrain_state_dict={}
            for key in pretrained_state_dict.keys():
                # print(key[7:10])
                if key[12] in ["1","2","3","4"]:
                    temp_pretrain_state_dict["features."+str(int(key[12])+3)+key[13:]]=pretrained_state_dict[key]
                elif key[7:10]=="bn1":
                    temp_pretrain_state_dict["features.1."+key[10:]]=pretrained_state_dict[key]
                else:
                    print(key+" is not found")
            pretrained_state_dict=temp_pretrain_state_dict
    ...
    for i in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            save_folder='models/mytrain/iresnet18'
            ...
            if acc > 0.87 :
                torch.save({'iter': i,
                            ...
                            os.path.join(save_folder, "epoch"+str(i)+"_acc"+str(acc)+".pth"))
                print('Model saved.')
